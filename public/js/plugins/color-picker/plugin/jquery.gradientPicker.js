...
(function( $ ) {
	...
	function GradientSelection($el, opts) {
		...
		$("body").bind("click", this.docClicked);
		...
		this.$el.draggable({
			...
			stop: function(event,ui){
				...
				if(mousey >= 500){
					...
					$(this).parent().parent().find(".gradientPicker-close").click();
				}
				this.stop;
			},
	...
	function _ControlPtConfig($parent, opts) {
		//color-chooser
		// this.$el = $('<div class="gradientPicker-ptConfig" style="visibility: hidden"></div>');
		// $parent.append(this.$el);
		var $cpicker = $parent;
	...
	ControlPtConfig.prototype = {
		...
		removeClicked: function() {
			...
			$(".gradientPicker-close").css("visibility", "hidden");
			this.hide();
		}
	};
