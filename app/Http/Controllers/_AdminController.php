...
//    	$groups = DB:table
	...
	public function update( Request $request ) {
		...
		//$filename = $get_selected->thumbnail;
		/*if ( Input::file( 'image_thumb' ) ) {

			$image = Input::file( 'image_thumb' );
			$path  = public_path( 'uploads/thumbnails' );
			if ( $filename != 'logo.jpg' ) {
				...
				$filename = substr( encrypt( $request->category_name ), 0, 25 ) . '.' . $image->getClientOriginalExtension();
				if ( file_exists( $path . "/" . $filename ) ) {
					$filename = substr( encrypt( $request->category_name ), 0, random_int( 10, 24 ) ) . '.' . $image->getClientOriginalExtension();
				}
