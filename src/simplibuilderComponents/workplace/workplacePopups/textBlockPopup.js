...
class TextBlockPopup extends Component {
  componentDidUpdate = (prevProps, prevState) => {
    const { textBlock, activeElement, autoCompleteOptions } = this.props;

    /* when user opens a new shortText */
    if (
      activeElement &&
      !textBlock[activeElement.id] &&
      activeElement.type === "textBlock"
    ) {
      this.props.updateData("SET_TEXTBLOCK", activeElement.id);
      return;
    }

    if (
      activeElement &&
      !prevProps.textBlock[activeElement.id] &&
      textBlock[activeElement.id] &&
      textBlock[activeElement.id]
    ) {
      this.props.updateData("SET_POPUP_ACTIVE_ELEMENT", {
        id: activeElement.id,
        answers: textBlock[activeElement.id]["answers"],
        generalDiscription:
          textBlock[activeElement.id]["ask"]["generalDiscription"],
        value: textBlock[activeElement.id]["ask"]["value"],
        type: "textBlock"
      });
    }
  ...
  */
  getNewWords = () => {
    ...
    words[`${activeElement.id}_textBlock_ask`] = {
      translation_id: null,
    ...
    activeElement["answers"].map((item, index) => {
      if (item.tag) {
        words[`${activeElement.id}_textBlock_tag_${index}`] = {
          translation_id: null,
          content: item.tag,
          language: selectedForm.default_lang.id
        };
      }
      words[`${activeElement.id}_textBlock_disc`] = {
  ...
  getWords = (value, key) => {
    ...
    getRequest(
      ...
      this.props.updateData("SET_ACTIVE_ITEM", {
        ...
        item: { id: `${activeElement.id}_textBlock_${key}` }
      });
    });
  };
  ...
  submit = () => {
    ...
    if (isValid) {
      ...
      elements.splice(0, 0, {
        ...
        generalDiscription: activeElement.generalDiscription,
        id: activeElement.id
      });
      ...
      this.props.updateData("SHOW_TEXTBLOCK", false);
        ...
        <PopUp show={show}>
          ...
          <div className="popup-body">
            ...
            <TextareaAutosize
              ...
              value={
                activeElement && activeElement.type === "textBlock"
                  ? activeElement["value"]
                  : ""
              }
              ...
              <ClickOutsideOptionsList
                ...
                show={
                  ...
                  activeAutoCompleteItem.id ===
                    `${activeElement.id}_textBlock_ask`
                }
            ...
            <div className="shorttext-options-conainer">
              <ClickOutsideOptionsList
                ...
                show={
                  ...
                  activeElement &&
                  activeAutoCompleteItem.id ===
                    `${activeElement.id}_textBlock_generalDiscription`
                }
                options={autoCompleteOptions ? autoCompleteOptions : []}
                renderKey="content"
...
const mapStateToProps = state => {
  return {
    ...
    show: state.workplacePopups.textBlock,

    textBlock: state.workplaceElements.textBlock,
...
export default connect(mapStateToProps, mapDispatchToProps)(TextBlockPopup);
